...
void main()
{
    ...
    float metallic = mrSample.b*u_metalFactor;
    ...
    vec3 diffuseContrib =(1.0-metallic)*(1.0 - F)*diffuse(baseColor.xyz);
    vec3 specContrib = F * G * D / (4.0 * NdotL * NdotV);
    
    // Obtain final intensity as reflectance (BRDF) scaled by the energy of the light (cosine law)
    vec3 color = NdotL * u_LightColor * (diffuseContrib + specContrib);

    vec3 diffuseColor=baseColor.xyz*(1.0-F)*(1.0-metallic);
    ...
    float ao = texture2D(mrSample, v_UV).r;
#endif
    color = mix(color, color * mrSample.r, u_OcclusionStrength);
